...
// EMBER GLASS: Menú móvil luminoso y cálido
export function MobileMenu({ onClose, onNavClick }: MobileMenuProps) {
  const menuItems = [
    { id: 'servicios', label: 'Servicios' },
    { id: 'precios', label: 'Planes y Precios' },
    { id: 'faq', label: 'Preguntas Frecuentes' },
    { id: 'contact', label: 'Contactar', highlight: true },
  ];
      ...
      <div className="flex flex-col w-full max-w-sm space-y-3 relative z-10">
        {menuItems.map((item, idx) => (
          <motion.button
            ...
            className={`
              w-full py-4 text-base font-medium rounded-2xl transition-all duration-300
              ${item.highlight 
